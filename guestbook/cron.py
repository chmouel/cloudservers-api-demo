...
class ScheduledTask(object):
    ...
    def get_status(self, ttype):
        ...
        if not bCi:
            ...
            return
        return "%s:%s" % (self.mclient.get("demo-cron-%sCurrentImage" % (ttype)),
    ...
    def main(self):
        opparser = optparse.OptionParser(usage="cron.py [options]")
        ...
        opparser.add_option('-K', '--hard-cleanup',
                            dest="hard_cleanup",
                            action="store_true",
                            help="Will cleanup everything including servers and images."
                            )
    ...
    def cleanup(self):
        print "Cleaning up:"
        s = ["demo-cron-backupCurrentImage", "demo-cron-restoreCurrentImage", "demo-cron-restoreStatus", "demo-cron-restoreStatus", "demo-cron-end", "demo-cron-start"]
        for t in s:
            ...
            if nh == sh and nm == sm:
                ...
                if not iscreated:
                    ...
                    if doingIt:
                        ...
                        if not i_have_printed:
                            i_have_printed=True
                            print "I was going to backup but they are already stored in images so I am going to pass on this one."
                ...
                if self.options.verbose:
                    ...
                    if not iscreated and now >= dtstart:
                        ...
                        if now > dtstart:
                            print "Will start backup tomorrow at %s." % dtstart.strftime("%H:%M:%S")
                            self.sleep_time = 3600
                        else:
                            print "Will start backup in %s" % timeuntil(dtstart)
                            self.sleep_time = 15
                time.sleep(self.sleep_time)
